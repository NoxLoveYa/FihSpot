...
export function Navbar({ mapType, onMapTypeChange, search }: NavbarProps) {
  ...
  // Profile chip and FihSpot label: no background (bare).
  const bare =
  ...
  return (
    <header className="fixed inset-x-0 top-0 z-[1200] flex flex-wrap items-center justify-between gap-x-1 gap-y-2 border-b border-slate-200/70 bg-white px-2 py-2 shadow-soft dark:border-slate-700 dark:bg-slate-800 lg:pointer-events-none lg:flex-nowrap lg:gap-2 lg:border-none lg:bg-transparent lg:px-3 lg:py-3 lg:shadow-none dark:lg:bg-transparent">
      <div className="flex min-w-0 items-center gap-1 lg:pointer-events-auto lg:gap-1.5">
        <button
          onClick={() => navigate('/')}
          title={t('app.title')}
          className={`${bare} sm:rounded-xl sm:px-2`}
        >
      ...
      <div className="order-3 flex w-full min-w-0 items-center gap-1 lg:order-none lg:pointer-events-auto lg:w-auto lg:flex-1">
        <button
